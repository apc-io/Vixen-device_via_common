...
#ifndef lint
static char rcsid[] = "$Id: chat.c,v 1.2 2001/08/27 07:51:22 philipc Exp $";
#endif
 ...
 */
	...
	while ((arg = ARG(argc, argv)) != NULL) {
	    ...
	    if ((arg = ARG(argc, argv)) != NULL){
						    if (verbose)
			fprintf (report_fp, "chat_send \"%s\" \"%d\"  \"%s\"...\n", __FUNCTION__,__LINE__,arg);
 ...
 */
	if (get_string (expect)){
				    if (verbose)
			fprintf (report_fp, "expect1 \"%s\" \"%d\"  \"%s\"...\n", __FUNCTION__,__LINE__,expect);

	    return;
		}else{
				    if (verbose)
			fprintf (report_fp, "expect2 \"%s\" \"%d\"  \"%s\"...\n", __FUNCTION__,__LINE__,expect);
			}
/*
