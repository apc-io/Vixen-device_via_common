...
ExifEntry* swExif::create_maker_tag(ExifData *exif, ExifIfd ifd, ExifTag tag, unsigned int strlen)
{
	void *buf;
	ExifEntry *entry;
	unsigned int size;
	...
	size = exif_format_get_size (EXIF_FORMAT_ASCII) * strlen;
...
void swExif::exif_write_datetimestamp(ExifData *exif, const char *string)
{
	...
	//ALOGW("year = %d , mon = %d day = %d",tm_data.tm_year + 1900,tm_data.tm_mon + 1,tm_data.tm_mday);
	//ALOGW("hour = %d , min = %d , sec = %d",tm_data.tm_hour,tm_data.tm_min,tm_data.tm_sec);
}
...
int swExif::exif_write_rational(ExifData *exif, ExifIfd ifd, int tag, const char *string)
{
	...
	if (c) {
		...
		//ALOGW("numerator = %d , denominator = %d",val.numerator,val.denominator);
	} else {
		val.denominator = 1;
...
int getManufacturer(char *value)
{   
   int len = property_get("ro.product.manufacturer", value, NULL);
   if(len <= 0)
   {
      strcpy(value,"WonderMedia");
   }
   return strlen(value);
}
...
{    
   int len = property_get("ro.product.model", value, NULL);
   if(len <= 0)
   {
      strcpy(value,"MID001");
   }
...
int swExif::createswexif(void)
{
	...
	if ((width == 0) || (height == 0)) {
		ALOGE("Picture : %d x %d ",width,height);
		rc = -1;
	}
	//ALOGW("\n\nPicture : %d x %d ",width,height);
	...
	if (mHasthumbnail == true) {
		...
		if ((twidth == 0) || (theight == 0)) {
			ALOGE("Thumbnail : %d x %d ",twidth,theight);
			rc = -1;
		}
	}
	//ALOGW("Thumbnail : %d x %d ",twidth,theight);
	if (rc)
		goto errout;
	//
	...
	//string = "WonderMedia";		
	len = getManufacturer(str);		
	entry = create_maker_tag(exif, EXIF_IFD_0, EXIF_TAG_MAKE, strlen(str)+1);
	strncpy ((char *)entry->data, str, entry->size);

	//entry = init_tag(exif, EXIF_IFD_0, EXIF_TAG_MODEL);
	//string = "MID001";
	len = getModel(str);
	entry = create_maker_tag(exif, EXIF_IFD_0, EXIF_TAG_MODEL, strlen(str)+1);
	strncpy ((char *)entry->data, str, entry->size);

	entry = init_tag(exif, EXIF_IFD_0, EXIF_TAG_DATE_TIME);

	entry = init_tag(exif, EXIF_IFD_0, EXIF_TAG_ORIENTATION);
	exif_set_short(entry->data, FILE_BYTE_ORDER, (short)orientation);
	...
	if (mHasthumbnail == true) {	
		...
		entry = init_tag(exif, EXIF_IFD_1, EXIF_TAG_ORIENTATION);
		exif_set_short(entry->data, FILE_BYTE_ORDER, (short)orientation);
	...
	if (mExifLength > 2048) {
		ALOGE(" mExifLength > 2048 ");
		goto errout;
	}
//ALOGW("Exif  len = %d , mThumbnailSize = 0x%x",data_len,mThumbnailSize);
