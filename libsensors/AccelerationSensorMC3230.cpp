...
AccelerationSensorMC3230::AccelerationSensorMC3230()
...
int AccelerationSensorMC3230::enable(int32_t, int en)
{
    int flags = en ? 1 : 0;
    int err = 0;
    if (flags != mEnabled) {
...
/*
int AccelerationSensorMC3230::enableOrientation(int en)
{
    int flags = en ? 1 : 0;
    int err = 0;
...
*/
int AccelerationSensorMC3230::setDelay(int32_t handle, int64_t ns)
{
    if (ns < 0)
        return -EINVAL;
    ...
    if (mEnabled /*|| mOrientationEnabled*/) {
        ...
        #if SENSOR_DEBUG
        ALOGD_IF(SENSOR_DEBUG,"AccelerationSensorMC3230::setDelay] delay =%d\n",delay);
        #endif
    }
...
int AccelerationSensorMC3230::readEvents(sensors_event_t* data, int count)
{
    if (count < 1)
        return -EINVAL;
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        // userspace using EVIOCGABS
            ...
            ALOGE("AccelerationSensorMC3230: unknown event (type=%d, code=%d)",
                    type, event->code);
        }
        mInputReader.next();
    }
...
void AccelerationSensorMC3230::processEvent(int code, int value)
{
	int x,y,z;
	...
	z=  0xff&(value>>16);
	#if SENSOR_DEBUG
	ALOGD_IF(SENSOR_DEBUG, "rx=%x,ry=%x,rz=%x,xyz=%x", x,y,z,value);
	#endif
	if (x&0x80)
	{
		x = x - 256;
	...
	#if SENSOR_DEBUG
	ALOGD_IF(SENSOR_DEBUG, "x=%x,y=%x,z=%x", x,y,z);
	#endif

	
    switch (code) {
        case EVENT_TYPE_ACCEL_X:
            mPendingEvent.acceleration.x = x * GRAVITY_EARTH / LSG_MC3230;
            mPendingEvent.acceleration.y = y * GRAVITY_EARTH / LSG_MC3230;
            mPendingEvent.acceleration.z = z * GRAVITY_EARTH / LSG_MC3230;
            break;
