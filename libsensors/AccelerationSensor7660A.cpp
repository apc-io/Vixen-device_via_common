...
AccelerationSensor7660A::AccelerationSensor7660A()
...
int AccelerationSensor7660A::enable(int32_t, int en)
{
    int flags = en ? 1 : 0;
    int err = 0;
    if (flags != mEnabled) {
...
/*
int AccelerationSensor7660A::enableOrientation(int en)
{
    int flags = en ? 1 : 0;
    int err = 0;
...
*/
int AccelerationSensor7660A::setDelay(int32_t handle, int64_t ns)
{
    if (ns < 0)
        return -EINVAL;
...
int AccelerationSensor7660A::readEvents(sensors_event_t* data, int count)
{
    if (count < 1)
        return -EINVAL;
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        // userspace using EVIOCGABS
            ...
            ALOGE("AccelerationSensor7660A: unknown event (type=%d, code=%d)",
                    type, event->code);
        }
        mInputReader.next();
    }
...
void AccelerationSensor7660A::processEvent(int code, int value)
{
	/*int x,y,z;
	...
	*/
    ...
    switch (code) {
        case EVENT_TYPE_ACCEL_X:
            mPendingEvent.acceleration.x = value * GRAVITY_EARTH / LSG_MMA7660A;
            //mPendingEvent.acceleration.y = y * GRAVITY_EARTH / LSG_MMA7660A;
            //mPendingEvent.acceleration.z = z * GRAVITY_EARTH / LSG_MMA7660A;
            break;
        case EVENT_TYPE_ACCEL_Y:
            mPendingEvent.acceleration.y = value * GRAVITY_EARTH / LSG_MMA7660A;
            break;
        case EVENT_TYPE_ACCEL_Z:
            mPendingEvent.acceleration.z = value * GRAVITY_EARTH / LSG_MMA7660A;
            break;
    }
}
